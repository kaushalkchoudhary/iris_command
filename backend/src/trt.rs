...
unsafe extern "C" {
    ...
    fn trt_get_input_dims(
        runner: *mut TrtRunnerOpaque,
        n: *mut c_int,
        c: *mut c_int,
        h: *mut c_int,
        w: *mut c_int,
...
impl TrtRunner {
    ...
    pub fn build_engine(onnx_path: &str, engine_path: &str) -> Result<()> {
        ...
        let res = unsafe {
            trt_build_engine(c_onnx.as_ptr(), c_engine.as_ptr())
        };
    ...
    pub fn input_dims(&self) -> Result<(i32, i32, i32, i32)> {
        ...
        let res = unsafe {
            trt_get_input_dims(self.ptr, &mut n, &mut c, &mut h, &mut w)
        };
    ...
    pub fn output_dims(&self) -> Result<(i32, i32, i32, i32)> {
        ...
        let res = unsafe {
            trt_get_output_dims(self.ptr, &mut n, &mut c, &mut h, &mut w)
        };
    ...
    // INFERENCE
    // ============================
    /// Synchronous inference (TensorRT manages CUDA internally)
    pub fn infer(&self, input: &[f32], output: &mut [f32]) -> Result<()> {
        let res = unsafe {
            trt_infer(self.ptr, input.as_ptr(), output.as_mut_ptr())
        };
