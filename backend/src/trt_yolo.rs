...
use opencv::{
    core::{BorderTypes, Mat, Scalar, Size},
    imgproc,
...
fn preprocess(frame: &Mat, input_w: i32, input_h: i32) -> Result<(Vec<f32>, Letterbox)> {
    ...
    for y in 0..input_h as usize {
        for x in 0..input_w as usize {
            ...
            input[base] = data[idx] as f32 / 255.0;
            input[base + hw] = data[idx + 1] as f32 / 255.0;
            input[base + 2 * hw] = data[idx + 2] as f32 / 255.0;
        }
    }
...
fn decode_yolo(output: &[f32], num_classes: usize, lb: &Letterbox) -> Result<Vec<RawDet>> {
    ...
    for i in 0..num {
        ...
        let mut x1 = (x - w * 0.5 - lb.pad_x) / lb.scale;
        let mut y1 = (y - h * 0.5 - lb.pad_y) / lb.scale;
        let mut x2 = (x + w * 0.5 - lb.pad_x) / lb.scale;
        let mut y2 = (y + h * 0.5 - lb.pad_y) / lb.scale;
